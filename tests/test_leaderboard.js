...
assert.strictEqual(lb1.getAllTimeBest(), 350, 'Legacy high score 350 should be migrated to allTime');
...
const res1 = lb1.recordScore(400, { rank: 'Tactician', clearsCount: 15 }, now1);
...
assert.strictEqual(lb1.getAllTimeBest(), 400);
assert.strictEqual(lb1.getWeeklyBest(now1), 400);
assert.strictEqual(lb1.getTodayBest(now1), 400);
console.log('✅ PASS: Record milestone flags set correctly on higher score');

// 4. Lower Score on Same Day
const res2 = lb1.recordScore(250, { rank: 'Calculator' }, now1);
...
assert.strictEqual(lb1.getTodayBest(tomorrow), 0, 'Tomorrow starts with 0 daily best');
assert.strictEqual(lb1.getWeeklyBest(tomorrow), 400, 'Sunday is same ISO week (W38), so weekly best remains 400');
const resTomorrow = lb1.recordScore(300, { rank: 'Calculator' }, tomorrow);
...
assert.strictEqual(lb1.getTodayBest(nextMonday), 0, 'Next Monday starts with 0 daily best');
assert.strictEqual(lb1.getWeeklyBest(nextMonday), 0, 'Next Monday is Week 39, resets weekly best to 0');
assert.strictEqual(lb1.getAllTimeBest(), 400, 'All-time best remains 400');
const resNextWeek = lb1.recordScore(320, { rank: 'Calculator' }, nextMonday);
...
for (let i = 1; i <= 15; i++) {
  lb1.recordScore(100 + i, { rank: 'Novice' }, nextMonday);
}
const history = lb1.getHistory();
